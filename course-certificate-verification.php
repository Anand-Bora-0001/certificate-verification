 ...
 * Author URI: https://github.com/Anand-Bora-0001
...
register_deactivation_hook( __FILE__, 'course_certificate_Anand-Bora-0001_certificate_onDeactivation' );
...
function course_certificate_certificate_search_form(){ 
    ...
    if( isset($_POST['code_data']) ){
        ...
        global $wpdb;
        $rows = $wpdb->get_results( "SELECT * FROM AB_course_certificates where certificate_code = '$code'"); 
        if( !empty($rows) ){
        $output .= '<h1 class="rs-heading">Search Result</h1>
            ...
            $output .= '<div class="danger">No result found against this code <strong>'.$code.'</strong></div>';
        } 
    } 
    $output .= '</div>';
    return $output;
}
add_shortcode( 'get_certificate_search_form' , 'course_certificate_certificate_search_form' ); // http://localhost/wordpress/index.php/certificate/?certificate=....

    function auto_search_certificate_by_url() {
        if (isset($_GET['certificate'])) {
            $certificate_code = sanitize_text_field($_GET['certificate']);
            ?>
            <script type="text/javascript">
                document.addEventListener('DOMContentLoaded', function () {
                    if (!sessionStorage.getItem('certificateSearchPerformed')) {
                        const searchField = document.querySelector('.cf-field');
                        const searchButton = document.querySelector('.cf-btn');
    
                        if (searchField && searchButton) {
                            searchField.value = "<?php echo esc_js($certificate_code); ?>";
                            sessionStorage.setItem('certificateSearchPerformed', 'true'); // Prevent loop
                            searchButton.click();
                        }
                    }
                });
...
add_action('wp_footer', 'auto_search_certificate_by_url');
